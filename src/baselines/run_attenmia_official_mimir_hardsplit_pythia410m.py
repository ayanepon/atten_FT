...
DEFAULT_RUN_DIR = "models/mimir_wikipedia_hardsplit_lora_ft_lr1e-4_epoch5_pythia410m"
DEFAULT_OUTPUT_DIR = "results/attenmia_official_mimir_hardsplit_pythia410m"
SEED = 42
EPS = 1e-12
...
def load_hardsplit_targets(args) -> pd.DataFrame:
    ...
    if args.n_per_group > 0:
        targets = (
            targets.groupby("group", group_keys=False)
            .apply(lambda x: x.sample(n=min(args.n_per_group, len(x)), random_state=args.seed))
...
def run_one(spec: ComparisonSpec, targets: pd.DataFrame, model, tokenizer, args) -> pd.DataFrame:
...
def main() -> None:
    ...
    for spec in comparison_specs(args):
        ...
        summaries.append(run_one(spec, targets, model, tokenizer, args))
