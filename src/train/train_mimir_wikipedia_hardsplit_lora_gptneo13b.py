# -*- coding: utf-8 -*-
"""
LoRA fine-tuning on the MIMIR Wikipedia hard split for EleutherAI/gpt-neo-1.3B.
...
The split/training pipeline is reused from train_mimir_wikipedia_hardsplit_lora.py:
  PT      = MIMIR Wikipedia member
  FT      = first half of MIMIR Wikipedia nonmember
  Unseen  = second half of MIMIR Wikipedia nonmember

Example:
  HF_TOKEN=... CUDA_VISIBLE_DEVICES=0 python3 train_mimir_wikipedia_hardsplit_lora_gptneo13b.py

By default this script reuses the already-saved Pythia split CSVs, so GPT-Neo
is fine-tuned on exactly the same FT samples as the Pythia run.

If the Pythia split directory is elsewhere:
  python3 train_mimir_wikipedia_hardsplit_lora_gptneo13b.py \
    --pythia-split-data-dir /path/to/mimir_wikipedia_hardsplit_lora_ft/data
"""
...
DEFAULT_OUTPUT_DIR = "models/mimir_wikipedia_hardsplit_lora_ft_lr1e-4_epoch5_gptneo13b"
...
def resolve_pythia_split_data_dir(explicit_dir: str) -> Path:
    ...
    raise FileNotFoundError(
        "Pythia split CSVs were not found. Pass --pythia-split-data-dir with the directory "
...
def make_existing_pythia_splits(split_data_dir: Path):
    ...
    print(f"Using existing Pythia split CSVs from: {split_data_dir}")
    return pt, ft, unseen
