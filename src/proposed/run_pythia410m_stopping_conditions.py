# -*- coding: utf-8 -*-
"""Run Pythia-410M attention-update extraction for 20/50/100 and early stopping."""
...
MODEL_ROOT = Path(
    os.environ.get(
        ...
        "models/mimir_wikipedia_hardsplit_lora_ft_lr1e-4_epoch5_pythia410m",
    )
)
OUTPUT_ROOT = Path(
    os.environ.get(
        "PYTHIA410M_STOPPING_OUTPUT_ROOT",
        "results/mimir_wikipedia_hardsplit_stopping_condition_pythia410m",
    )
)
...
def resolve_adapter_dir() -> Path:
    ...
    raise FileNotFoundError("Pythia-410M LoRA adapter was not found. Tried:\n" + tried)
